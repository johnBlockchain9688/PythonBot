...
UNIVERSAL_V3_ROUTER_ABI =json.loads("""
...
class Uniswap(Defi):
    def __init__(self, wallet_address, private_key, chain_name: str):
        self.chain = chain_name
        self.blockChain = Chain(chain_name)
        self.w3 = self.blockChain.web3
        ...
        #Router
        self.router_address=Web3.to_checksum_address(os.getenv(chain_name.upper()+"V3_UNISWAP_ROUTER_ADDRESS"))
        if self.router_address is None:
                raise ValueError("Uniswap Router address not set. Check .emv file")
        self.router = self.w3.eth.contract(address=self.router_address, abi=UNIVERSAL_V3_ROUTER_ABI)
        #Permit
        self.permit2_address=Web3.to_checksum_address(os.getenv(chain_name.upper()+"_UNISWAP_PERMIT2_ADDRESS"))
        ...
        if stuck_nonce is not None:
            logger.info("stuck transaction detected")
            self.cancel_transaction(stuck_nonce)
        else:
            logger.info("No stuck transactions to cancel")
            time.sleep(1)
    ...
    def approve_permit2(self, token_address, amount):
        ...
        logger.info(f"Permit2 token approve transaction hash: {tx_hash.hex()}")
    ...
    def make_trade(self, from_token_name: str, to_token_name: str, amount: float, fee, slippage):
        """
        Execute an exact input swap using Universal Router with RouterCodec.
        
        Args:
            from_token_name (str): Name of token to swap from
            to_token (str): Name of token to swap to
        ...
        start_token= Token(self.chain,from_token_name)
        amount_in_wei=start_token.get_wei_amount(amount)
        end_token =Token(self.chain,to_token_name)

        from_token = Web3.to_checksum_address(start_token.address)
        to_token = Web3.to_checksum_address(end_token.address)

        decimals_in = start_token.decimals
        logger.info(f"Input amount in wei: {amount}")
        logger.info(f"Token decimals: {decimals_in}")

        balance =   start_token.balance_of(self.address)
        logger.info(f"Token balance in wei: {balance}")
        logger.info(f"Token balance in token: {balance / (10 ** decimals_in)}")
        
        if balance < amount_in_wei:
            raise Exception(f"Insufficient balance. Have: {balance / (10 ** decimals_in)}, Need: {amount / (10 ** decimals_in)}")
        ...
        if not has_permit2_allowance:
            ...
            if not approval_success:
                raise Exception("Failed to get Permit2 approval")

            time.sleep(2)  # Wait for approval to b"e mined
        else:
        ...
        if not permit_data or not signed_message:
            raise Exception("Failed to create permit signature")


        logger.info(f"permit_data: {permit_data}")
        logger.info(f"signed_message: {signed_message}")
        logger.info(f"amount_in_wei: {amount}")
        ...
        max_token_price=end_token.get_token_price()+end_token.get_token_price()*slippage
        min_amount_out =round(amount_in_wei/max_token_price)
        # slippage 0.5% fee 0.25% ，minimum received 99.25 %
        minimum_out = int(amount_in_wei * (1 - Decimal("0.005") - Decimal("0.0025")))
        ...
        tx = {
            ...
            "nonce": self.w3.eth.get_transaction_count(self.account.address),
            "gas": int(gas_params['estimated_total_wei']),  # Use estimated gas from gas_params
            "maxFeePerGas": int(gas_params['max_fee_per_gas']),
            "maxPriorityFeePerGas": int(gas_params['max_priority_fee_per_gas']),
